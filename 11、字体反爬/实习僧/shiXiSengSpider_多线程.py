...
df = pd.DataFrame()
base_url_format = 'https://www.shixiseng.com/interns?page={}&type=intern&keyword={}&area=&months=' \
...
def get_all_pages(keyword):
    # 获取总页数
    start_url = f'https://www.shixiseng.com/interns?keyword={keyword}&city=%E5%85%A8%E5%9B%BD&from=menu'
    ...
    for item in items:
        data = {}
        data['title'] = item.css('.intern-detail__job>p>a::attr(title)').get()
...
def run(page, keyword):
    r = requests.get(base_url_format.format(page, keyword), headers=headers)
    if r.status_code == 200:
        sel = parsel.Selector(r.text)
        # 获取职位详情页url
        urls = sel.css('div.intern-detail__job a::attr("href")').extract()
        print(f"获取到{keyword}当前页的所有job对应的链接{urls}")
        for url in urls:
            get_job_detail(url)
    else:
        print(r.status_code)
...
def main():
    keyword = "Java"
    pages = get_all_pages(keyword)
    print(f'{keyword}类的职业一共{pages}页')
    # with ThreadPoolExecutor() as pool:
    #     for page in range(1, pages + 1):
    #         pool.submit(run, page, keyword)
    for page in range(1, pages + 1):
        run(page, keyword)

    df.to_excel('职位信息3.xlsx', index=False)
