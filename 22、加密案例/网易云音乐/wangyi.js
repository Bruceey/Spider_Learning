...
e8e = {
    cookie: true,
    data: "rid=R_SO_4_190072&threadId=R_SO_4_190072&pageNo=1&pageSize=20&cursor=-1&offset=0&orderType=1",
    method: "GET",
    noescape: undefined,
    onerror: ƒ(),
    onload: ƒ(),
    query: "rid=R_SO_4_190072&threadId=R_SO_4_190072&pageNo=1&pageSize=20&cursor=-1&offset=0&orderType=1",
    type: "json",
}
...
// 一级
var bsK3x = function (czA1x) {
    var m8e = [];
    czA1x.forEach((item, index) => {
        m8e.push(emj[item])
    });
    return m8e.join("")
};
...
function getdata(i8a) {

    // i8a["csrf_token"] = "";
    // i8a是json字符串
    var bWv6p = asrsea(i8a, bsK3x(["流泪", "强"]), bsK3x(md), bsK3x(["爱心", "女孩", "惊恐", "大笑"]));
    // e8e.data = {
    ...
    var d = {}
        ...
        lg2x: function (a) {
            var c = this.i8a
                ...
                , k = this.blockSize
                , b = j / (4 * k)
                , b = a ? u.ceil(b) : u.max((b | 0) - this.GX1x, 0);
            a = b * k;
            j = u.min(4 * a, j);
            if (a) {
                for (var q = 0; q < a; q += k)
...
CryptoJS.lib.Cipher || function (u) {
    var p = CryptoJS
        ...
        createEncryptor: function (e, a) {
            return this.create(this.FF1x, e, a)
        },
        createDecryptor: function (e, a) {
            return this.create(this.bsc2x, e, a)
        },
        init: function (e, a, b) {
    ...
    d.BlockCipher = v.extend({
        ...
        reset: function () {
            ...
            var a = this.cfg
                , b = a.iv
                , a = a.mode;
            if (this.XU5Z == this.FF1x)
                var c = a.createEncryptor;
            else
                c = a.createDecryptor,
                    this.GX1x = 1;
            this.eV0x = c.call(a, this, b && b.words)
        },
        qB4F: function (a, b) {
            this.eV0x.processBlock(a, b)
        },
        mG2x: function () {
...
(function () {
        ...
        var H = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54]
            , d = d.AES = p.extend({
            lJ2x: function () {
                for (var a = this.J8B, c = a.words, d = a.sigBytes / 4, a = 4 * ((this.ZX6R = d + 6) + 1), e = this.bsA3x = [], j = 0; j < a; j++)
                    if (j < d)
                        e[j] = c[j];
                    else {
                        var k = e[j - 1];
            ...
            encryptBlock: function (a, b) {
                this.DV0x(a, b, this.bsA3x, t, r, w, v, l)
            },
            decryptBlock: function (a, c) {
                var d = a[c + 1];
                a[c + 1] = a[c + 3];
                a[c + 3] = d;
                this.DV0x(a, c, this.bsE3x, b, x, q, n, s);
                d = a[c + 1];
                a[c + 1] = a[c + 3];
                a[c + 3] = d
            },
            DV0x: function (a, b, c, d, e, j, l, f) {
                for (var m = this.ZX6R, g = a[b] ^ c[0], h = a[b + 1] ^ c[1], k = a[b + 2] ^ c[2], n = a[b + 3] ^ c[3], p = 4, r = 1; r < m; r++)
